...
const ESTABLISHMENT_TYPES = [
	...
	{ label: 'Cafetería', value: 'COFFEE_SHOP' },
	{ label: 'Bar', value: 'BAR' },
	...
	{ label: 'Frutería', value: 'FRUIT_SHOP' },
	{ label: 'Carnicería', value: 'BUTCHER_SHOP' },
	...
	{ label: 'Papelería', value: 'STATIONERY_STORE' },
	{ label: 'Juguetería', value: 'TOY_STORE' },
	{ label: 'Tienda de Electrónicos', value: 'ELECTRONICS_STORE' },
	...
	// Servicios
	{ label: 'Peluquería', value: 'HAIR_SALON' },
	{ label: 'Barbería', value: 'BARBER_SHOP' },
	...
	{ label: 'Consultorio Médico', value: 'MEDICAL_OFFICE' },
...
export default function CompleteProfileScreen({
	...
	const [userRole, setUserRole] = useState<'BENEFICIARY' | 'ESTABLISHMENT' | null>(null);
	...
	const [establishmentType, setEstablishmentType] = useState('RESTAURANT'); // Valor por defecto válido
	...
	const [showEstablishmentTypeModal, setShowEstablishmentTypeModal] = useState(false);
	...
	const handleCompleteProfile = async () => {
		...
		try {
			...
			// No navegamos manualmente - el App.tsx se encargará automáticamente
			// cuando user.isActive cambie a true
			// La navegación condicional en App.tsx detectará el cambio y mostrará Home
		} catch (err) {
		...
		<SafeAreaView style={{ flex: 1, backgroundColor: '#fff' }}>
			...
			>
				...
				>
					...
					>
						<Text style={{ color: '#fff', fontWeight: 'bold', fontSize: 16 }}>1</Text>
					</View>
					<View
						style={{
							flex: 1,
					...
					>
						<Text style={{ color: '#fff', fontWeight: 'bold', fontSize: 16 }}>2</Text>
					</View>
				</View>
				...
				{step === 1 && (
					<View>
						...
						{formErrors.name && (
							<FeedbackMessage type="error" message={formErrors.name} visible={true} />
						)}
						...
						<TouchableOpacity
							...
							style={{
								...
								padding: 20,
								marginBottom: 15,
								backgroundColor: userRole === 'BENEFICIARY' ? '#E8F5E9' : '#f9f9f9',
							}}
						>
							<Text
								style={{
						...
						<TouchableOpacity
							...
							style={{
								...
								padding: 20,
								marginBottom: 30,
								backgroundColor: userRole === 'ESTABLISHMENT' ? '#E8F5E9' : '#f9f9f9',
							}}
						>
							<Text
								style={{
				...
				{step === 2 && (
					<View>
						...
						<Input
							label={
								userRole === 'BENEFICIARY' ? 'Nombre *' : 'Nombre del Establecimiento *'
							}
							placeholder={
								userRole === 'BENEFICIARY' ? 'Nombre *' : 'Nombre del Establecimiento *'
							}
						...
						<View style={{ flexDirection: 'row', gap: 10, marginBottom: 15 }}>
							<View style={{ flex: 0.7 }}>
								...
								>
									<Text style={{ fontSize: 14, color: '#333' }}>{countryCode}</Text>
								</TouchableOpacity>
							</View>
							<View style={{ flex: 1.3 }}>
								<Input
							...
							<>
								...
								>
									...
									<Text style={{ color: '#333', fontSize: 14, marginTop: 4 }}>
										{ESTABLISHMENT_TYPES.find((e) => e.value === establishmentType)
			...
			<Modal visible={showDocumentTypeModal} transparent animationType="slide">
				<SafeAreaView style={{ flex: 1, backgroundColor: 'rgba(0,0,0,0.5)' }}>
					...
					>
						...
						>
							<Text style={{ fontSize: 18, fontWeight: 'bold' }}>Tipo de Documento</Text>
						</View>
							...
							renderItem={({ item }) => (
								<TouchableOpacity
									...
									style={{
										...
										borderBottomWidth: 1,
										borderBottomColor: '#eee',
										backgroundColor: documentType === item.value ? '#E8F5E9' : '#fff',
									}}
								>
									<Text
										style={{
			...
			<Modal visible={showCountryCodeModal} transparent animationType="slide">
				<SafeAreaView style={{ flex: 1, backgroundColor: 'rgba(0,0,0,0.5)' }}>
					...
					>
						...
						>
							<Text style={{ fontSize: 18, fontWeight: 'bold' }}>Código de País</Text>
						</View>
							...
							renderItem={({ item }) => (
								<TouchableOpacity
									...
									style={{
										...
										borderBottomWidth: 1,
										borderBottomColor: '#eee',
										backgroundColor: countryCode === item.value ? '#E8F5E9' : '#fff',
									}}
								>
									<Text
										style={{
			...
			<Modal visible={showEstablishmentTypeModal} transparent animationType="slide">
				<SafeAreaView style={{ flex: 1, backgroundColor: 'rgba(0,0,0,0.5)' }}>
					<View
						style={{
							flex: 1,
							...
							renderItem={({ item }) => (
								...
								>
									<Text
										style={{
											fontSize: 16,
											color: establishmentType === item.value ? '#4CAF50' : '#333',
											fontWeight: establishmentType === item.value ? '600' : '400',
										}}
									>
										{item.label}
									</Text>
