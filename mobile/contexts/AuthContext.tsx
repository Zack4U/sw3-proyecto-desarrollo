...
export const AuthProvider: React.FC<AuthProviderProps> = ({ children }) => {
		...
		(
			...
			try {
				...
				return {
					...
					role: decoded.role || 'BENEFICIARY',
					isActive: decoded.isActive ?? true,
					isVerified: true,
					establishmentId: decoded.establishmentId || establishmentId, // Extraer del token
				};
			} catch (error) {
	...
	const refreshTokens = useCallback(async (): Promise<boolean> => {
		try {
			...
			// Extraer información del usuario desde el nuevo token
	 ...
	 * Crea usuario con isActive = false
	 */
	const registerBasic = useCallback(
		async (data: { email: string; password: string; confirmPassword: string }) => {
			try {
				...
				const response = await authService.registerBasic(data);
		...
		async (data: CompleteProfileData) => {
			try {
				...
				const user: User = {
					...
					isVerified: true,
					establishmentId: response.establishmentId, // Incluir ID de establecimiento si existe
				};
				await saveUser(user);
				dispatch({
					type: 'LOGIN_SUCCESS',
